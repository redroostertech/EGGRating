...
public class EggRating: NSObject {
  ...
  @objc public static func promptRateUsIfNeededMessageExt(in viewController: MSMessagesAppViewController) {
        if shouldPromptForRating {
            self.promptRateUs(in: viewController)
            lastRemind = Date()
        }
    ...
    @objc public static func promptRateUsMessageExt(in viewController: MSMessagesAppViewController) {
